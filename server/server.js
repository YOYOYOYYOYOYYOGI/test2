...
import { analyzeWithProviders, publicProviderStatus, synthesizeWithProvider } from './providers/index.js';
...
function safePath(urlPath) { const raw = decodeURIComponent(urlPath.split('?')[0]); const requested = raw === '/' ? '/dashboard.html' : raw; const relative = requested.startsWith('/extension/') ? requested.slice('/extension/'.length) : requested.slice(1); const resolved = path.resolve(ROOT, 'extension', relative); if (!resolved.startsWith(path.resolve(ROOT, 'extension'))) return null; return resolved; }
