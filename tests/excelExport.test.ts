...
describe('dynamic Excel columns', () => {
  it('builds field columns (in field order), Qty columns, then Delivery Charge + Total', () => {
    ...
    expect(headers).toEqual(['Order Number', 'Customer Name', 'WhatsApp Number', 'Address', 'Pincode', 'Night Cream Qty', 'Face Serum Qty', 'Day Cream Qty', 'Order Date', 'Delivery Charge', 'Total', 'Previous Order Number', 'Previous Sequence Order Number']);
  });
  ...
  it('respects sheet header mappings and per-field columnHeader overrides', () => {
    ...
    expect(excelHeaders(c)).toEqual(['Order No.', 'Client Name', 'Order Date', 'Delivery Charge', 'Total', 'Previous Order Number', 'Previous Sequence Order Number']);
  });
  ...
  it('uses only included fields and falls back to active products when products.included is empty', () => {
    ...
    expect(excelHeaders(c)).toEqual(['Customer Name', 'Alpha Qty', 'Order Date', 'Delivery Charge', 'Total', 'Previous Order Number', 'Previous Sequence Order Number']);
  });
  ...
  it('never emits duplicate columns even when names collide', () => {
    ...
    expect(excelHeaders(c)).toEqual(['Customer Name', 'Customer Name Qty', 'Order Date', 'Delivery Charge', 'Total', 'Previous Order Number', 'Previous Sequence Order Number']); // product col suffix keeps it unique
  });
  ...
  it('reflects new custom fields and product list changes automatically', () => {
    ...
    // Order Date + delivery/total trail every export, then the two previous-
    // order reference columns (both plain text).
    expect(headers.indexOf('Hair Mask Qty')).toBe(headers.length - 6);
    expect(headers[headers.length - 5]).toBe('Order Date');
    expect(headers[headers.length - 4]).toBe('Delivery Charge');
    expect(headers[headers.length - 3]).toBe('Total');
    expect(headers[headers.length - 2]).toBe('Previous Order Number');
    expect(headers[headers.length - 1]).toBe('Previous Sequence Order Number');
  });
});
